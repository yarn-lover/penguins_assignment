...
However, there are other variables recorded in the dataset which might affect body mass - for instance, if penguins are sexually dimorphic, sex may also impact body mass. Species may also play a role: some species may have different relationships between body mass and flipper length than others. Therefore, before conducting a statistical test of the association of penguin flipper length and body mass, I will create an exploratory plot, to look at the relationship of penguin flipper length and body mass between different species.
...
#load the function definition files we will use, from the folder 'functions'in this R project
...
##we will turn it into a new dataset (penguins_clean), to avoid overwriting the raw data (so we can return to it if there are errors in the pipeline)
#we will use functions from 'cleaning.R' file in 'functions' folder. This enables the code here to be quick and easy to understand, with greater detail present in the 'cleaning.R' file if needed. 
...
##new dataset will include only the variables we are interested in for this exploratory figure: flipper length, body mass, species, and sex
#use a pipeline combining a subset function with a function to remove NAs from the data (as points with NA are not useful to the analysis)
...
#save the exploratory figure as a .svg to the 'figures' folder, using a function from the 'plotting' file
save_exploratory_figure_svg(penguins_explore,
                              "figures/fig01_vector.svg",
                              size = 15, scaling = 1)


#save the exploratory figure as a .png to the 'figures' folder, using a function from the 'plotting' file
...
This graph shows up many potentially interesting questions that could be statistically tested. Since the assignment does not allow box plots, the remaining test available is to test the relationship between the two numerical variables: flipper length and body mass. 
Since species and gender appear that they could be influencing body mass and flipper length, to avoid any effects caused by species or sex, I will control for this in my statistical analysis, by only testing one species and one sex. I will test female Gentoos, as it appears from the exploratory plot that this may be one of the stronger associations. 
...
I will first filter the data to contain only the flipper length and body mass of Gentoo penguins; then calculate the sample correlation coefficient (r); and then test if this is significantly different from 0. This test will use alpha = 0.05 as the significance level.
...
##SUBSET THE DATA
#subset the data so it only contains female Gentoo penguins, and contains only the variables of interest (flipper length & body mass) and removes observations containing NA, using functions from the cleaning file
...
#make a new variable: 'p_value', to store the p_value output in, for use in the results figure later on
p_value <- cor.test(penguins_female_gentoo$flipper_length_mm, penguins_female_gentoo$body_mass_g)$p.value
...
Correlation coefficient of sample: 0.48
